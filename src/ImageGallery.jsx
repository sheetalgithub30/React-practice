...
function ImageGallery() {
    ...
    function handleClick(index){
      setSelected(img[index]);
    }

  return (
    <div>
        <h1><u>Click on an image !!</u></h1>
        <div id="images" className='my-6'>
          {
            img.map((src,index)=>{
             return <img src ={src} key={index} onClick={(e)=>{handleClick(index)}}></img>
            })
          }
        </div>
         {
          <div className=' flex justify-center items-center'>
            <img src={selected} className='rounded-2xl my-6'></img>
